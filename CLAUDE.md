...
- **`/product`** (`skills/product*/`) — PRODUCT, the product-definition meta: eight chained stages, one document each, all under `docs/` — `brief` → `competitors` → `prd` → `features` → `roadmap` → `requirements` → `architecture` → `engineering`. The chain exists so the MVP is not an opinion: it is the top release slice of a story-mapped feature inventory built on a real competitive landscape. State at `./.claude/product/state.md` (stage status, lock, verdict, open clarifications, drift log). Gated by default — each stage ends with an approve/revise decision and locks on approval; `--fast` runs the chain unattended, but **never past a `kill` or `needs-clarification` verdict from stage 0**, because six stages on a dead idea is the exact waste the brief exists to prevent. Greenfield/brownfield is auto-detected: with source present the stages read the code and backfill what the product already does. Re-invocation resumes; all eight done, it switches to **maintain mode** and reports drift instead of rewriting.

  Three rules run the whole family. **Altitude is enforced** — stages 0–4 are WHAT/WHY with no technology in them at all (a named stack in the PRD is a decision that skipped the architecture stage), stage 5 is testable behavior, stages 6–7 are HOW. **An unknown has three dispositions and only one is a question for the user**: `[NEEDS RESEARCH]` is homework the child owes and clears before its stage ends (research that comes up empty becomes a cited absence, never a question); `[NEEDS CLARIFICATION]` is what only the user knows or decides, asked one at a time as *choices* (multi-select where several answers can be true) before the gate opens, and an open one blocks approval short of a recorded override; a *deferred validation* is what nobody can know until the product exists, carried forward as a planned check, never a marker. An assumption that reads like a finding corrupts every downstream stage. **Traceability is a defect check, not decoration** — every `R-NN` names its `F-NN` and its release, and the audit runs both directions, because requirements-with-no-feature catches invention while features-with-no-requirement catches omission. Each stage ends with an analyze pass (contradictions, gaps, orphans, altitude violations, open markers) before the gate.

  `product-competitors` is dual-role and declares its storage mode per invocation: **matrix mode** (the SQLite kitchensink shared with `feature-tracker`, all verbs, marketing exports) where `marketing.sqlite` already exists, **document mode** (`docs/competitors.md` + research files) everywhere else — it never creates that DB in a repo that lacked it. Its research engine is three agents in parallel (site crawler, feature extractor, positioning analyst). Stages 6–7 carry the standing **free-and-permissive default**: every dependency gets a `License:` and `Cost:` verdict, and copyleft or paid needs a written argument naming the free equivalent and whether the obligation even attaches — then it is the user's call at the gate, never an executor's quiet default. The family **stops at documents**: handing them to `/ip` is the user's move.
- **`/user-docs`** (`skills/user-docs/`) — keeps end-user product documentation true each iteration: adds new features' operating instructions, updates changed behavior, deletes removed features' docs. Downstream of `/product` and deliberately separate: `/product` defines what to build, `/user-docs` documents how to operate what shipped.
- Every iterate plan ends with three standing finisher steps appended by the planner: `/dev-makefiles` maintenance, `/testmaster` run (fast+standard), then `/user-docs` sync. The planner also scans every plan for testable behavior and routes it through `/testmaster-derive` before writing validations.
...
- **`/iterate-triage`** (`skills/iterate-triage/`) — walk up to a stale terminal and get one short answer. Reads real state (plans, branch, uncommitted work, blockers) and reports **only what is broken**, in a fixed shape — `complete: x of y`, `broken: z of y`, `Detail:` (≤150 words per problem), `Fix:` (≤50 words per problem) — never what went right. Every `Fix` ends in exactly one human act, planned end to end by triage first: a ready script the user runs with `pbcopy < /tmp/<plan>-p<N>.sh`, or `launch problem N` when a human must sign in / click / decide, after which triage does everything up to that one act. Things it already fixed get one line: `done — committed as <sha>`. **Reaching triage is itself the finding**: `/iterate` is supposed to land an all-green plan on main by itself, so a feature branch sitting idle means automation failed — triage names what. Commits loose work immediately and without asking (a branch commit is not a merge, carries no risk, and the auto-commit hook does NOT cover it — snapshots never move HEAD); finishes an *owed* merge when the plan is genuinely all-green, because that is an obligation, not a decision; never merges anything that is not.
- **`/iterate-conductor`** (`skills/iterate-conductor/`) — the conductor: works the whole queue unattended. `/iterate` runs *a* plan; the conductor runs *the queue* — picks the next plan, lets `/iterate` drive it, handles the ending, sweeps again. Blockers get an escalation ladder (a **different approach**, never more retries — the 5-cycle cap stays, because an unattended loop is what ticked a dead plan for 13 hours) plus a per-plan wall-clock box so one stubborn plan can't eat the night. What genuinely cannot be cleared is **parked in place**: the plan keeps its own file, branch and history, gets `status: blocked-on-operator`, and the conductor moves to the next one. You unblock it from a **second session** while the conductor keeps working — the lock is per-plan, so that is not a conflict; the shared working tree is, so environment blockers need no branch and code blockers use a `git worktree`. `/iterate-triage` marks it `status: unblocked` (**cyan**) when cleared, and the conductor takes cyan plans first. Red plans get one cheap re-test per completed sweep cycle, never every sweep. A sweep that finds nothing startable **winds the conductor's own tick down**: 5m → 60m `watching` while something could still change by itself, then `stood-down` — one `PushNotification` naming what the operator must supply, its cron cancelled and verified, plans left red where they are. Standing down is finishing, not stopping: `start`, `resume`, or `/iterate-triage` clearing a blocker re-arms it. Nothing that can only change because a human decides to change it is ever polled for. Imports all open GitHub/GitLab issues as a single plan (capped per sweep, never re-imported, read-only on the forge). Verbs: `start|stop|pause|resume|run|status|kill` — `stop` and `pause` let the current plan finish, `kill` is the only one that interrupts. State at `./.claude/iterate/conductor.md`, which also holds an optional `conductor-schedule:` — same grammar as the launch schedule but **intersected** with it, so it narrows the conductor's own hours without ever widening past what `/iterate-rules` allows. `/ic schedule <rule>` delegates to `/iterate-rules`; the conductor reads `policy.md` and never writes it. Starting it is the standing authorization the launch keyword asks for; the launch *schedule* still governs when.
- **`/iterate`** (`skills/iterate/`) — executes a saved plan autonomously to completion, looping via `/loop` until every validation passes or it hits a genuine blocker. Dispatches one subagent per team on teamed plans. `/iterate pause [<name>]` stops cleanly at the next turn boundary — loop cancelled and verified, loose work committed on the branch, `status: paused` set (and the conductor paused with it if it was driving); `/iterate resume [<name>]` or an explicit `/iterate <name>` continues from the first unchecked step. A no-arg tick never resumes a paused plan. Every plan keeps a **running-resources ledger** (`## Running resources`: each VM / container / background process / agent a step leaves up, with its exact stop and start commands, `scratch` or `deliverable`); `pause` and `/ic kill` stop every entry and sweep `izmachine list` for leaks from this project's other plans, `resume` starts what pause stopped, and every ending stops `scratch` entries — a parked plan never keeps the machine busy.

**Every team carries a model tier.** The Teams table's `Model` column names it, `/iterate` passes it straight through at dispatch, and `/iterate-planner` rule 30 is the only place the mapping lives: `fable` coordinates, `opus` builds, `sonnet` reads and reports, `haiku` extracts. Bare aliases, not pinned ids, so a model release doesn't mean editing twelve skills. The coordinator's own model is a recommendation only — a skill cannot change the model of the session running it.

**Visual verification runs in a subagent and the image never comes back.** A screenshot is 100–160k tokens and, because every request re-sends the whole conversation, it is paid again on every later request in that lane — twelve of them were 26.6% of plan galago's entire token cost. A `sonnet` subagent looks at the file, answers one question, and dies with the image; crop before capturing, and when the question is numeric (does it overflow, what is the width) measure it instead. Nothing gets skipped — only the artifact stops being permanent.

Plans name their feature branch but do not create it — the branch is born at execution, so the status line keeps reading `main ✔` until real work starts, and the statusline's `⚙️` segment (one letter per plan) carries plan state instead of the branch name doing it badly. **Colour is state, case is teaming**: green executing, yellow planned, red blocked, cyan unblocked-and-ready, magenta paused, dim closed — and UPPERCASE means `teamed: true` (one subagent per team at dispatch), lowercase means flat (one lane, at the launching session's model, since a flat plan has no Teams table and therefore no per-team `Model` column). `⚙️ Bc` is two plans: a teamed one executing, a flat one planned. The script is `~/.claude/statusline-command.sh`, which is not tracked in any repo.
...
`iterate-brainstorm` and the six aliases all carry the same `version:` — the
...
**Blocked on one feature never holds the branch.** Every blocked ending runs the **landing test**: the blocked clauses are the only unmet ones, the suite is green on a bare box with the blocked feature's tests skipping, that feature is inert on main (gated or unreferenced), and nothing half-wires a user-visible path. Passing, `/iterate` merges the green work as if all-green and rolls only the parked feature to a new plan on a new branch; failing, the branch stays unmerged with the ⚠ line. Plans therefore state `Depends on:` against the packages a predecessor produces, never "stage N merged to main".

**Free and permissive is the standing default** across the whole family: no purchase, no subscription, no metered account; MIT/BSD/Apache-2.0/ISC/MPL-2.0 and never GPL/LGPL/AGPL/SSPL — unless an argument survives being written down (name the free equivalent, say what it can't do, say whether the obligation attaches at all — linking a library is not the same as running a container). A paid or copyleft choice is then the user's to make through a `human-gate` step or `/ibs`, never a default the planner takes or an executor's workaround. Plans emit `License:` and `Cost:` constraints for everything they adopt, and those constraints are the only authorization.

**The substitution test** runs before anything is called an access dependency: a vendor named in a requirement (Fastmail, Stripe, a hosted dashboard) is a dependency on that vendor only when the vendor is the deliverable. When the requirement is an open standard, standing up a conformant server is a *creation step* — Dovecot for IMAP, Stalwart/Apache James/Cyrus for JMAP, MinIO for S3, Keycloak for OIDC — and there is nothing to preflight. `/iterate-planner` applies it in the access preflight and writes validations against the local server; `/iterate` applies it again before declaring any access wall, substituting the endpoint (same assertions, logged) rather than blocking; the conductor's access ladder starts there.

Aliases: `/ip` → iterate-planner, `/i` → iterate, `/in` → iterate-notes, `/it` → iterate-triage, `/ibs` → iterate-brainstorm, `/ic` → iterate-conductor (all in `skills/`). `/ip`, `/in` and `/it` delegate via the Skill tool; `/i`, `/ibs` and `/ic` cannot (their targets' flags block the Skill tool), so they read and follow the target's SKILL.md directly — the user typing `/i`, `/ibs` or `/ic` is the explicit invocation the flag reserves.

`/iterate`, `/iterate-brainstorm`, `/iterate-conductor`, `/i`, `/in`, `/it`, `/ip`, `/ibs`, and `/ic` carry `disable-model-invocation: true`. For `/iterate` it is side effects (autonomous execution, PR merges); for `/iterate-brainstorm` it is that a decision session is something the user opens deliberately, never something natural language trips into; for `/iterate-conductor` it is the same side effects as `/iterate` but across the whole queue, unattended. `/iterate-planner` deliberately does NOT carry the flag: `/ip` delegates to it via the Skill tool, and that flag blocks Skill-tool delegation entirely. Plan state lives at `./.claude/iterate/plans/<name>.md` in whichever project they're run from; live status/dashboard for that state is `iterate-run` (`src/cmd/iterate-run/`, this repo's own Go binary — `iterate-run serve` for the web dashboard, `iterate-run status`/`timeline`/`tokens` for the CLI).
