...
Argument: <empty = full adopt | --seed-only | --covers-only | <test-id...>>. `$1` is its first word; `$ARGUMENTS` is the whole thing.
...
invoke `$testmaster-run` explicitly over the newly registered tests. Tiers come from measured `avg_ms` and nothing else. A test that has not run is `unmeasured`, never guessed into a tier.
