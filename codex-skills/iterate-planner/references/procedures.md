...
1. Resolve the target plan (named, else current).
2. If `running:` is a fresh timestamp (within 90s — a live `$iterate` run): refuse — "a run is live on `<name>`; stop the run first (under Codex, pause the Automation you created — this skill armed nothing), then close." Otherwise proceed (a stale heartbeat or `running: false` is the normal case here).
3. This skill arms no scheduler under Codex, so there is nothing to cancel — but if `loop-mechanism:` names a user-managed Automation, tell the user to pause it so nothing re-fires against an archived plan.
