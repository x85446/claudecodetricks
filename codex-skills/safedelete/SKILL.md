...
Argument: [folder containing _original-bundles, or --originals X --against Y]. `$1` is its first word; `$ARGUMENTS` is the whole thing.
...
1. **Verify first (never delete yet).** Run:
   ```bash
   python3 ~/.agents/skills/safedelete/safedelete.py "$ARGUMENTS"
   ```
...
4. **If SAFE:** tell the user it's verified, then **ask for confirmation** before deleting. On confirmation, re-run with `--trash`:
   ```bash
   python3 ~/.agents/skills/safedelete/safedelete.py "$ARGUMENTS" --trash
   ```
