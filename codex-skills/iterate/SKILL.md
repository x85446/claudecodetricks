...
3. **Dispatch every currently-ready, not-yet-dispatched team as parallel subagents, in one instruction** — state plainly that you are spawning one agent per ready team to work concurrently, name each one, and give each its full self-contained brief (step 4). **Pass each team's `Model` cell from the Teams table as that dispatch's `model`**, verbatim — the plan already decided the tier from the work (see `$iterate-planner` rule 30), so this is a pass-through, never a judgment you re-make here. A team whose row has no `Model` cell inherits, and that is worth one Status/Log line: an unstamped team is a planning gap, not a default. Set that row's `Status` to `in-progress` and write the plan file **before** dispatching, so the record survives even if this turn dies mid-flight. Codex runs them in parallel, waits until all requested results are available, and returns a consolidated response — you do not poll and you do not need a completion notification. <!-- codex-port: subagent orchestration rewritten in plain language; Codex exposes no fixed spawn/wait tool schema to skill authors — verify against https://learn.chatgpt.com/docs/agent-configuration/subagents -->
...
29. **Visual verification runs in a subagent, and the image never comes back.** A screenshot is the single most expensive thing that can enter a context: 100–160k tokens, and — because every request re-sends the whole conversation — it is paid again on every later request in that lane. Measured on plan galago: twelve screenshots were 26.6% of the entire run's token cost, and the most expensive was read once and rode along for 177 requests. Spawn a `sonnet` subagent with one instruction — *look at `<file>` and answer `<the exact question>`* — and keep only its prose answer. Crop to the region in question before capturing. When the question is numeric, **measure it instead**. None of this skips a check; only the artifact stops being permanent.

30. **The coordinator's own model is a recommendation, never something to switch mid-run.** A skill cannot change the model of the session running it, so if this run is on a lower tier than coordination wants, say so once in the Status/Log and carry on. Never claim to have switched, never spawn a subagent to "be the coordinator", and never stop over it.
