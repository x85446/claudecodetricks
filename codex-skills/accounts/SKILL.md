...
| **Global** | `~/.agents/skills/accounts/known.md` | Personal / cross-project (your machines, your everyday accounts) |
...
3. Read `~/.agents/skills/accounts/known.md` if it exists.
...
4. **Ask where to store**, by asking the user to choose from a short numbered list. Options offered depend on context:
   - **Project** (`<project-root>/.claude/data/accounts.md`) — recommended when the info is project-scoped
   - **Global** (`~/.agents/skills/accounts/known.md`) — recommended when the info is yours across projects
...
- **Never write to a global store from inside a project unless explicitly asked.** When ambiguous, ask by asking the user to choose from a short numbered list.
...
- **Global file (`~/.agents/skills/accounts/known.md`) is NOT committable.** It's personal. Stays on this machine only.
