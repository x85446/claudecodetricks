...
Argument: <adopt | maintain | prune | run [tier] | report | nightly | status>. `$1` is its first word; `$ARGUMENTS` is the whole thing.
...
0. **derive** ("derive tests for <requirement>", "should this have a test", "what tests does this need") → invoke `$testmaster-derive` explicitly, args verbatim. Also the entry point `$iterate-planner` uses on every plan.
