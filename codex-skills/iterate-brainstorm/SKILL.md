...
The argument is the decision you need help with, e.g. "need some help deciding on a communications protocol". `$ARGUMENTS` is the whole thing.
...
2. **Never plan and never execute.** No 1a/1b pairs, no steps, no validations, no auto-resume cron. When the user wants a plan, the answer is the numbered summary plus "hand that to `$ip`" — not a plan written here.
