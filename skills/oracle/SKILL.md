---
name: oracle
description: The project's buzzword glossary. Surfaces the **5W+H** (Who, What, When, Where, Why, How) for any registered buzzword or buzzphrase the user might mention — subprojects (incusmagic, P64), tools, internal jargon, named workflows ("mgmt.gravhl.com new-service workflow"), domain terms. **Use IMMEDIATELY** when the user (a) mentions a likely buzzword that this project might have a glossary entry for, (b) asks "what is X", "tell me about X", "what does X do", "where is X", "remind me about X", "we're going to work on X", "how do I use X", "explain X", (c) shares new info about a topic ("X is a tool that...", "X lives at /path/...", "X is owned by Y"), or (d) invokes `/oracle` directly. The skill stores knowledge two places: `./.claude/data/oracle.md` (project-local entries) and `~/.claude/skills/oracle/known.md` (global / cross-project entries). On lookup, both are merged; project wins on conflict. The oracle is the lazy-loaded primer for everything the project assumes you already know.
argument-hint: <show|list|add|discover|update|remember|move|forget|harvest> [buzzword] [args]
---
...
| Global | `~/.claude/skills/oracle/known.md` | Cross-project buzzwords (tools you use everywhere — incusmagic, ssh-config, your home network names, etc.) |
...
**On write: default to PROJECT.** Don't pop an AskUserQuestion. The user opts into global explicitly via:
...
| `add` | `/oracle add <buzzword> [global\|project]` | Interactive 5W+H entry creation (user dumps knowledge via AskUserQuestion). **Defaults to PROJECT**. Add `global` to override. |
...
Don't pop an AskUserQuestion for store choice unless genuinely ambiguous (e.g., user said both "global" and "this project" in the same message).
...
5. **For non-matches that look intentional** (user invoked `/oracle X` directly, or asked "what is X"): offer to add an entry. One-line `AskUserQuestion`: "No oracle entry for `<X>`. Add one now?" Default no — don't push.
...
2. Global store: `~/.claude/skills/oracle/known.md`.
...
2. **Determine the target store** (no AskUserQuestion):
...
3. Walk through 5W+H interactively. For each W: one AskUserQuestion. Skip fields the user leaves blank.
...
3. **Research phase** (run all that apply, in parallel where possible):
   ...
   - Check `~/.ssh/config.d/<buzzword>` and `~/.claude/skills/<buzzword>/` directly.
...
7. **Ask via AskUserQuestion** — three options:
...
8. **If Edit in typora**:
   ...
   - AskUserQuestion: "Done editing — save what's in the file, or discard?"
...
4. Show the diff (existing entry vs proposed). Confirm via AskUserQuestion.
...
4. Show the entry that will be moved + confirm via AskUserQuestion: "Move `<buzzword>` from <source> to <destination>?" Default yes.
5. On confirm:
...
1. **Determine which store** holds the entry:
   ...
   - If buzzword exists in both → ask via AskUserQuestion which to remove (or both).
2. Show the entry that will be removed.
3. Confirm via AskUserQuestion: "Remove `<buzzword>` from <store>?" Default no.
...
3. Present as a list (AskUserQuestion, multiSelect). Default: none checked.
