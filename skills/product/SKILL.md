---
name: product
description: "PRODUCT — the product-definition meta. Takes an idea to a staged, engineering-ready definition through eight chained documents in docs/: brief, competitors, prd, features, roadmap, requirements, architecture, engineering. Routes ALL product-definition work here; the meta picks the child and holds the gate."
when_to_use: "Use for any product-definition work: \"define a new product\", \"start a new product\", \"write the PRD\", \"we need a product spec\", \"what's our MVP\", \"scope the MVP\", \"which features are in v1\", \"stage the roadmap to 3.0\", \"plan v1 v2 v3\", \"write the requirements\", \"spec this out for engineering\", \"document the architecture\", \"who are our competitors\", \"competitive landscape\", \"write a PRFAQ\", \"working backwards\", \"is this idea worth building\", \"product brief\", \"feature inventory\", \"backfill docs for what we already built\". Also /product. NOT for end-user operating instructions (that is /user-docs) and NOT for executing the build (that is /iterate)."
