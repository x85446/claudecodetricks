---
name: md-tester
description: Use when the user invokes /md-tester to manage test spec markdown files, add or update or delete TM entries, process ai|feedback annotations, run integration tests, or fix failing tests.
disable-model-invocation: true
argument-hint: [run|add|update|feedback|delete|create] [file] [TM-NNN]
---

## What This Skill Does

Manages test spec `.md` files that follow the TM-{NNN} format. Handles full CRUD on test entries, executes tests via their `run:` blocks, and auto-fixes failures in a retry loop.
...
2. Read `README.md` inside `test_docs_path` to load:
   - The **entry format** (TM-{NNN} template)
...
|---------|---------|--------|
| `run` | `/md-tester run provider TM-003` | Execute tests |
| `add` | `/md-tester add provider` | Add new TM entry |
| `update` | `/md-tester update aws-cli TM-005` | Fill in blank fields |
| `feedback` | `/md-tester feedback aws-cli.md` | Process ai\|feedback annotations |
| `delete` | `/md-tester delete provider TM-004` | Remove a TM entry |
...
Execute tests defined in TM entries.

### Steps

1. **Identify targets.** Parse arguments to determine which entries to run:
   - `run provider TM-003` — single entry in `provider.md`
...
2. **Skip unimplemented.** If a TM entry has no `run:` block, skip it and note it as "not yet implemented."
3. **Execute.** For each target entry, extract the `run:` bash block and execute it from the project root directory.
4. **Report results.** Show pass/fail status for each entry. Include the TM-{NNN} ID and short title.
...
Add a new TM-{NNN} entry to an existing test spec file.

### Steps

1. **Identify file.** The argument is the spec file name (without `.md` extension). Read the file from `test_docs_path`.
2. **Determine next TM number.** Scan existing entries to find the highest TM-{NNN} and increment by 1. Pad to 3 digits.
...
Fill in blank fields on existing TM entries by generating Go test code.

### Steps

1. **Identify targets.** Parse arguments to find which entries need updating.
   - `update aws-cli TM-005` — single entry
...
Process `ai | feedback:` annotations on TM entries (rule R-007).
...
3. **Report.** List each TM entry that was modified and what changed.

---

## Operation: `delete`

Remove a TM entry from a test spec file.

### Steps

1. **Identify the entry.** Parse `delete [file] TM-{NNN}`.
2. **Show the entry to the user.** Display the full TM entry content.
3. **Confirm deletion.** Ask the user to confirm before removing.
4. **Remove.** Delete the entire entry block (from `### TM-{NNN}` to the next `### TM-` or end of file).
5. **Do NOT renumber.** Leave gaps in TM numbers. Other entries may reference these IDs.
...
3. **Generate the file.** Create it in `test_docs_path` with:
   ...
   - Separator (`---`)
   - No initial TM entries (user adds them with `add`)
...
- **Don't renumber TM entries.** Gaps are fine; IDs may be referenced elsewhere.
