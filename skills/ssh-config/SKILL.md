...
argument-hint: <add|delete|move|sort|list|new-site|speedup> [args]
---
...
Prompt the user via AskUserQuestion for the minimum fields:
...
2. **Inspect existing controlmaster state.** Parse the option lines inside the block (case-insensitive):
   ...
   - If any of the three is present but not all (partial state) → ask via AskUserQuestion: "Partial speedup config exists. Replace with defaults?" Default yes. On yes: remove the existing controlmaster/controlpath/controlpersist lines (Step 4 will re-insert). On no: skip this host.
