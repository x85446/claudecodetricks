...
argument-hint: <what you want to do, e.g. "make alma 9 vm called X on H91" or "exec into Y" or "snapshot Z">
---

# /incus — incus VM/container management for Claude Code

Read [routing.md](routing.md) for the catalog of remotes, location → ssh-config-file map, and defaults. Re-read whenever defaults might have shifted.
...
# SKILL.md from ~/.claude/skills/incus/ or from the canonical repo):
...
3. If still ambiguous AFTER step 2, ASK the user once (AskUserQuestion). Then **record the answer** in `<cwd>/.claude/incus.md` for future sessions.
