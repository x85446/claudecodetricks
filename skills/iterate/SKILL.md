...
the sole executing plan); with nothing executing, print `nothing running` and
stop.

   **`pause`** — stop at the next turn boundary and leave the state safe to walk
   away from:
   - If `running:` is a fresh heartbeat (<90s), a tick is mid-step somewhere.
     Do not interrupt it: set the flag below and say `pausing — step <n>
     finishes, then nothing more runs`. That tick ends its turn as normal; the
     next one hits the guard in 0.5.
   - Set `status: paused: <UTC now> at step <n>/<m>` in the frontmatter.
     `phase: executing` stays — paused is a run state, not an ending.
   - Cancel the loop **exactly as a terminal exit does**: the mechanism in
     `loop-mechanism:`, result read, confirmed dead, field cleared. If the
     cancel comes back "not found", the loop was armed by another session and
     cannot be killed from here — leave the field and say so. Its ticks are
     no-ops while paused (0.5), which is safe, just not silent.
   - **If the conductor is driving this plan** (`conductor.md` has
     `enabled: true` and `current:` naming it), set `paused: true` there too
     and log it in both files. A paused plan under a running conductor is
     indistinguishable from a stalled one: the plan box would park it as
     blocked within 45 minutes. `resume` clears both.
   - **Stop what the plan left running** — every entry in `## Running
     resources` still up, then the leak sweep — exactly as "Running resources"
     describes. This is the half of pause that gives the machine back: a
     paused plan whose VMs keep running has not paused anything that costs.
   - Commit loose work on the feature branch, as `/iterate-triage` does — a
     paused run is exactly the idle branch that loses work. A real message,
     never "wip".
   - Touch nothing else: not the branch, not the tree, not `running:`. Log one
     line — `paused by operator at step <n> — <mechanism> cancelled, <k> files
     committed as <sha>, <j> resources stopped (<names>)` — and report the
     same line plus `resume with /iterate resume`.

   **`resume`** — remove `status: paused`, log `resumed by operator`, clear the
   conductor's `paused:` if the pause log line says it set it, then continue
   exactly as `/iterate <name>` does on an executing plan: re-check the branch,
   **start every resource pause stopped** (the `paused-by: pause` entries, in
   order, each verified up before the next), re-arm the loop, record the new
   `loop-mechanism:`, pick up at the first unchecked step. `/iterate <name>` on a paused plan does the same — an
   explicit human invocation is a resume. A no-arg tick never is (0.5).
...
3. **Dispatch every currently-ready, not-yet-dispatched team in ONE Agent tool call** — one `tool_use` block per team, launched together so they run concurrently. **Pass each team's `Model` cell from the Teams table as that dispatch's `model`**, verbatim — the plan already decided the tier from the work (see `/iterate-planner` rule 30), so this is a pass-through, never a judgment you re-make here. A team whose row has no `Model` cell inherits, and that is worth one Status/Log line: an unstamped team is a planning gap, not a default. Each is `run_in_background: true` (the default) — do not block this turn waiting on them. Set that row's `Status` to `in-progress` immediately on dispatch, so a status report never has to say just "still going" — it can say *which* teams are running.
...
30. **Visual verification runs in a subagent, and the image never comes back.** A screenshot is the single most expensive thing that can enter a context: 100–160k tokens, and — because every request re-sends the whole conversation — it is paid again on every later request in that lane. Measured on plan galago: twelve screenshots accounted for 26.6% of the entire run's token cost, and the most expensive one was read once and rode along for 177 requests.

    So dispatch a `sonnet` subagent with one instruction — *look at `<file>` and answer `<the exact question>`* — and keep only its prose answer. The image dies with the subagent. Crop to the region in question before capturing; a cropped panel measured 19x cheaper than the full page for the same verification. And when the question is numeric — does it overflow, what is the width, is the value right — **measure it instead**: a `scrollWidth`/`innerWidth` read costs ~300 tokens and cannot be misread the way a clamped or scaled screenshot can.

    None of this skips a check. Verification that was going to happen still happens; only the 150k-token artifact stops being permanent.

31. **The coordinator's own model is a recommendation, never something to switch mid-run.** Coordination — merging a team's log, judging "done", picking a different approach after a blocker — is the `fable` tier's work. But a skill cannot change the model of the session running it, so if this run is on a lower tier, say so once in the Status/Log and carry on. Never claim to have switched, never dispatch a subagent to "be the coordinator", and never stop over it.
