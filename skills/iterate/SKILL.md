...
4. **Each team's Agent prompt must be fully self-contained** (a fresh subagent has no memory of this conversation or this plan file beyond what you put in the prompt). Include, verbatim:
   ...
   - **Mandatory progress checkins, on a tight cadence — real content, not a guess.** Prefer running any operation likely to take more than ~1 minute in the background with its output streamed (e.g. via the Monitor tool or equivalent) rather than as a blind blocking call — most compilers, build systems, and test runners already emit incremental output (files compiled, tests passed) for free; streaming surfaces that without needing to touch the tool itself. Relay real content from that stream into the log **at least once a minute** (`<timestamp> step <N> in progress: <the latest real stream line, not a paraphrase>`) — this is what real observability looks like, not a heartbeat that just says "still going." **If you are writing a new script/tool as part of this step** (not just invoking an existing one), it must emit its own progress — a line per item or per batch when processing many items — so it's observable the same way; a silent loop over thousands of items is exactly the black-box case this exists to prevent. Only fall back to a blind blocking call with a pre-announced line when the operation genuinely can't be streamed or chunked (`<timestamp> starting: <what>, expect ~Nm` — use the Constraint's real number if one exists, never a made-up guess). The team must also log a line every time it finishes a step. Never go longer than a minute without a write, and never go silent until the terminal line.
