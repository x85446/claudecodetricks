...
4. **Each team's Agent prompt must be fully self-contained** (a fresh subagent has no memory of this conversation or this plan file beyond what you put in the prompt). Include, verbatim:
   ...
   - **Mandatory progress checkins, on a tight cadence — not just a final line.** The team must append a timestamped line to its log *every time it finishes a step*, and **at least every 2 minutes** while a step is still in flight (`<timestamp> step <N> done: <one line>` / `<timestamp> step <N> in progress: <what it's currently doing>`) — never go longer than that without a write, and never go silent until the terminal line. **Before starting any single operation expected to block longer than 2 minutes** (a build, a deploy, a long test run), log a line first stating what's about to run and an ETA if known (`<timestamp> starting: make app-build, expect ~10m`) — so a gap right after that line reads as "expected, mid-build" instead of "unexplained silence," to anyone reading the log including the coordinator's staleness check. This is what makes the log a real, inspectable progress trail instead of a black box, and it's what makes the staleness check below able to act quickly instead of tolerating long unverified quiet.
...
6. **Merge as soon as you know, not just on a poll.** Two ways you find out a team is done:
   ...
   - **Fallback: the next `/loop`/cron tick (every 1 minute)**, for the rare case a notification was missed (session restart, harness hiccup) — and, in practice, the thing you should actually rely on to catch a quiet team quickly, since notification timing isn't guaranteed. On every tick, check every outstanding team's (dispatched, not yet merged) scoped log file against three tiers, not two — silence up to a large threshold is not "nothing to do":
     - **Fresh** (a write within the last ~3 minutes, matching the team's own mandated ≤2-minute checkin cadence plus a small buffer) → on track. Leave it, don't act.
     - **Overdue** (no write in 3-10 minutes, no terminal line) → **don't wait passively — ping it.** Send a lightweight status-check message by its dispatch name (`<plan-name>-<team-name>`) asking it to report progress. This is cheap (a heartbeat costs nothing) and catches a problem — or confirms a long blocking operation is legitimately still running — well before it becomes a big silent gap. If it responds, or the log gets a fresh write, or the last line was a "starting: <long op>, expect ~Nm" line whose ETA hasn't elapsed yet, treat it as fine and don't escalate further this tick.
     - **Stale** (no write in 10+ minutes, no terminal line, AND unresponsive to a prior ping) → now, and only now, treat it as dead. Side-effecting work (writes, migrations, deploys) can't safely run twice — a second agent doing the same work could double-apply a change the first one already made, or corrupt a resource under concurrent access — so this tier requires having already tried the ping in the Overdue tier and gotten nothing back. Log "team `<name>` unresponsive after ping, no heartbeat for 10m+, treating as dead, re-dispatching" and dispatch a fresh one.
